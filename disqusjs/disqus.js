...
function DisqusJS(config) {
    ((window, document, localStorage, fetch, Promise) => {
        ...
        */
        const HTML_TPL_EL_HEADER = (num, title) => `<header class="dsqjs-header" id="dsqjs-header"><nav class="dsqjs-nav dsqjs-clearfix"><ul><li class="dsqjs-nav-tab dsqjs-tab-active"><span>${num} Comments</span></li><li class="dsqjs-nav-tab">${title}</li></ul><div class="dsqjs-order"><input class="dsqjs-order-radio" id="dsqjs-order-desc" type="radio" name="comment-order" value="desc" checked="true"><label class="dsqjs-order-label" for="dsqjs-order-desc" title="sort-by-decending-time">newest</label><input class="dsqjs-order-radio" id="dsqjs-order-asc" type="radio" name="comment-order" value="asc"><label class="dsqjs-order-label" for="dsqjs-order-asc" title="sort-by-ascending-time">oldest</label><input class="dsqjs-order-radio" id="dsqjs-order-popular" type="radio" name="comment-order" value="popular"><label class="dsqjs-order-label" for="dsqjs-order-popular" title="sort-by-grade">popular</label></div></nav></header>`;
        ...
        const HTML_TPL_EL_ASK_FOR_FULL = 'For better experience, you may enable proxy on disq.us | disquscdn.com | disqus.com and try to <a id="dsqjs-reload-disqus" class="dsqjs-msg-btn">load full Disqus mode</a> | <a id="dsqjs-force-disqus" class="dsqjs-msg-btn">force loading full Disqus mode</a>';
        ...
        function loadDisqus() {
            if (window.DISQUS) {
                ...
                $$(DISQUS_CONTAINER_EL_ID).innerHTML = `<div id="dsqjs"><section><div id="dsqjs-msg">full mode is loading... if it takes longer than usually, please enable proxy on disq.us | disquscdn.com | disqus.com or switch to <a id="dsqjs-force-dsqjs" class="dsqjs-msg-btn">simple Disqus mode</a></div></section>${HTML_TPL_EL_FOOTER}</div>`
        ...
        function checkDisqus() {
            $$(DISQUS_CONTAINER_EL_ID).innerHTML = `<div id="dsqjs"><section><div id="dsqjs-msg">Checking if Disqus is accessible...</div></section>${HTML_TPL_EL_FOOTER}</div>`;
        ...
        function loadDsqjs() {
            // DisqusJS 加载中信息
            msg(`Simple model is loading... ${HTML_TPL_EL_ASK_FOR_FULL}`);
            ...
            _get(url).then(({ data }) => {
                if (data.code === 0 && data.response.length === 1) {
                    ...
                    $$(DISQUS_CONTAINER_EL_ID).innerHTML = `<div id="dsqjs"><div id="dsqjs-msg">Simple mode is loading... ${HTML_TPL_EL_ASK_FOR_FULL}</div>${HTML_TPL_EL_HEADER(length, disqusjs.config.siteName)}<section class="dsqjs-post-container"><ul class="dsqjs-post-list" id="dsqjs-post-container"><p class="dsqjs-no-comment">Loading comments...</p></ul><a id="dsqjs-load-more" class="dsqjs-load-more dsqjs-hide">Load more</a></section>${HTML_TPL_EL_FOOTER}</div>`;
                    ...
                    msg('Current Thread hasn\'t been created. Switch to <a id="dsqjs-force-disqus" class="dsqjs-msg-btn">full Disqus mode</a>？')
                    $$('dsqjs-force-disqus').addEventListener(CLICK, useDisqus);
                } else {
                    throw new Error;
                }
            ...
            const getComment = (cursor = '') => {
                ...
                const unregisterListenerForSwitchTypeRadioAndGetMoreCommentBtn = () => {
                    ...
                    Array.prototype.slice.call($loadHideCommentInDisqus).forEach(i => i.removeEventListener(CLICK, checkDisqus));
                }
                ...
                const getCommentError = () => {
                    if (cursor === '') {
                        ...
                        // 在按钮上显示提示信息
                        $loadMoreBtn.innerHTML = 'Load failed, press to retry.';
                        // 重新在按钮上绑定 加载更多按钮
                ...
                const switchSortType = ({ target }) => {
                    ...
                    $$('dsqjs-post-container').innerHTML = '<p class="dsqjs-no-comment">Changing sorting type...</p>';
                    // 把 加载更多评论 隐藏起来
                ...
                _get(url).then(({ data }) => {
                    if (data.code === 0 && data.response.length > 0) {
                        ...
                        Array.prototype.slice.call($loadHideCommentInDisqus).forEach(i => i.addEventListener(CLICK, checkDisqus));
                        ...
                        // 当前没有评论，显示提示信息
                        msg(`You may not have access to Disqus, we enable simple mode for you. ${HTML_TPL_EL_ASK_FOR_FULL}`)
            ...
            const renderComment = (data) => {
                ...
                const renderPostItem = (s) => {
                    ...
                    let message = '';
                    if (s.isDeleted) {
                        message = `<small>This comment has been deleted.</small>`;
                    } else {
                ...
                const childrenComments = (data) => {
                    ...
                    children.map((comment) => {
                        ...
                        const hasMoreEl = comment.hasMore
                            ? `<p class="dsqjs-has-more">Switch to <a class="dsqjs-has-more-btn" id="load-more-${comment.comment.id}" data-more-id="comment-${comment.comment.id}">full Disqus mode</a> to display more comments</p>`
                            : '';
                ...
                parseCommentData(data).map((comment) => {
                    ...
                    let hasMoreEl = '';
                    if (comment.hasMore) {
                        hasMoreEl = `<p class="dsqjs-has-more">Switch to <a id="load-more-${comment.comment.id}">full Disqus mode</a> to show more replies</p>`
                    }
        ...
        function loadError(err) {
            console.log(err);
            msg('Unable to load Disqus normally，click <a id="dsqjs-reload-dsqjs" class="dsqjs-msg-btn">this</a> to retry, or <a id="dsqjs-reload-disqus" class="dsqjs-msg-btn">try to load full Disqus mode</a> ？');
        ...
        disqusjs.config = _extends({
            api: 'https://disqus.edlinus.cn/api/',
